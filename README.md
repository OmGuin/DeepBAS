...
- Simulation will parallelize across `num_workers` processes. Ensure sufficient cpu count. Each worker requires <10 mb of RAM.

`sim_dataMCval_1a.py`: Testing dataset generation. Arguments: num_workers

`sim_datagen.py`: Testing set generation for specific test cases.
- Manually edit test cases at bottom of file.
- For example, the following tuple:  (X, 2, (I_Mm, I_Mp), N_L, (W_M, W_M), D_L), describes a distribution with two species, one at 2000 counts/500μs and another at 3000 counts/500μs, low concentration ($1.4*10^{-11}$ M), both medium width ($\sigma$ = 500), and diffusion coefficeint of $10^{-11}$. X is the ID; it can be any number as long as all IDs are exclusive.
- saves arrival times in numpy array

`sim_dataMC_timevary.py`: Testing for running at different event counts. Arguments: num_workers

`utils.py`: Contains utility functions for generating distributions for all simulation scripts. 

## Deep Learning

`train_lim.py`: Train linear model. Arguments: learning rate, batch size, epochs, output bins, inital beta, save, progress bar
- Example usage: `python train_lim.py --lr=2e-4 --batch_size=32, --num_epochs=100, --output_bins=50, --init_beta=0.1`
...
`train_log.py`: Train log models. Same IO as linear.

`models.py`, `datasets.py`: Contain classes for models and datasets, respectively.

`CNN_LINEAR.ipynb`, `CNN_LOG.ipynb`: Notebooks for validation for linear and log models, respectively
...
- Outputs .par files needed for BAS & BAS preprocessing input