...
def SimPhotDiffFlowGL6(C_molar, Rp, sigma_b, alpha, D, totalTime, binDt, w0, axialFactor, includeBg, bgRate, beamShape, vFlow, rng, num_species, dist_type='ln', resFactor=10):
    # 1b) Concentration
    C_m3 = C_molar * 1e3
    ...
    b = compute_underlying_dist(Rp=Rp, dist_type=dist_type, sigma_b=sigma_b, alpha=alpha, rng=rng, Nres=Nres)
...
def run_one_sim(sim_num):
    ...
    with open(os.path.join(data_dir, f"GT{sim_num:07}.json"), "w") as f:
        json.dump(GT, f)
